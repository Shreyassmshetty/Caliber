...
import { Apple, Eye, EyeOff, Dumbbell, Sparkles, KeyRound, ArrowLeft, CheckCircle2 } from 'lucide-react';
import { motion } from 'motion/react';

export const Auth = () => {
  const { signup, login, googleLogin, forgotPassword, resetPassword, loading, error } = useApp();
  ...
  const handleRequestResetCode = async (e) => {
    ...
    if (res && res.success) {
      if (res.resetCodeDemo) {
        setGeneratedDemoCode(res.resetCodeDemo);
      }
      setForgotStep(2);
    }
  };
  ...
  const handleResetPassword = async (e) => {
    ...
    const success = await resetPassword(forgotEmail, resetCode, newPassword);
    if (success) {
      setForgotSuccessMessage("Your password has been reset successfully! You can now sign in.");
  ...
  return (
    <div className="min-height-[90vh] flex flex-col justify-center px-4 py-12 sm:px-6 lg:px-8 max-w-md mx-auto">
      <div className="text-center mb-8">
        ...
        >
          {isForgotMode ? <KeyRound className="w-8 h-8" /> : <Apple className="w-8 h-8" />}
        </motion.div>
          ...
          <div>
            {forgotSuccessMessage ? (
              <div className="text-center py-6 space-y-3">
                ...
                <h3 className="text-lg font-bold text-gray-800">Password Reset Complete!</h3>
                <p className="text-xs text-gray-600">{forgotSuccessMessage}</p>
                <p className="text-[11px] text-gray-400">Redirecting to sign in...</p>
              </div>
            ) : forgotStep === 1 ? (
              ...
              <form className="space-y-5" onSubmit={handleResetPassword}>
                ...
                {generatedDemoCode && (
                  <div className="bg-emerald-50 text-emerald-800 p-3.5 rounded-xl text-xs border border-emerald-200 space-y-1">
                    <p className="font-semibold flex items-center gap-1">
                      <span>🔑 Verification Code Generated:</span>
                      <span className="font-mono bg-emerald-200/80 px-2 py-0.5 rounded text-emerald-950 font-bold text-sm tracking-wider select-all">{generatedDemoCode}</span>
                    </p>
                    <p className="text-[11px] text-emerald-700">Enter this 6-digit code below to reset your password.</p>
                  </div>
                )}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="123456"
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm font-mono tracking-widest text-center text-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"
                  />
                </div>
